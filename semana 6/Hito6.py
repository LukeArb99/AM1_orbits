from numpy import linspace, zeros
from Lagrange import CR3BP, Lagrange_points,Stability_LP
import random
from Probema_Cauchy import Problema_de_Cauchy
from Esquemas_temporales import Embedded_RK
import matplotlib.pyplot as plt

## Datos
mu = 1.2151e-2

## inicial
N=int(10000)
t = linspace(0,10,N)
Np = 5

##condiciones iniciales
U_0 = zeros([Np, 4])

U_0[0, :] = [0.1, 0, 0, 0]
U_0[1, :] = [0.8, 0.6, 0, 0]
U_0[2, :] = [-0.1, 0, 0, 0]
U_0[3, :] = [0.8, -0.6, 0, 0]
U_0[4, :] = [1.01, 0, 0, 0]

LP = Lagrange_points(U_0, Np,mu)

print("Puntos de lagrange")
for i, point in enumerate(LP,start=1):
    print(f"LP{i}=",point)

# seleccion de puntos de lagrange
selected_point = input("Enter the number of the Lagrange point you want to select to study its stability: ")

try:
    selected_point_index = int(selected_point)

    # Check if the selected index is valid
    if 1 <= selected_point_index <= Np: #len(Np):
        selected_lagrange_point = LP[selected_point_index - 1]
        print(f"You have selected Lagrange Point {selected_point_index}: {selected_lagrange_point}")
    else:
        print("Invalid selection. Please enter a valid Lagrange point number.")
except ValueError:
    print("Invalid input. Please enter a number.")

## Calculos a partir del punto indicado
U0 = zeros(4)
U0[0:2] = LP[selected_point_index - 1,:]
perturbation = 1e-4*random.random()             
U0 = U0 + perturbation   

def F(U,t):
    return CR3BP(U,mu)

##Calculo de la solucion
U = Problema_de_Cauchy(F, t, U0, Embedded_RK)

##Estabilidad de los puntos
U0_stab = zeros(4)
U0_stab[0:2] = LP[selected_point_index - 1, :]
eingvalues = Stability_LP(U0_stab, mu)

##Ploteo

lagrange_point_names = ['L1', 'L2', 'L3', 'L4', 'L5']

fig, (ax1, ax2) = plt.subplots(1, 2)
ax1.plot(U[:, 0], U[:, 1], '-', color="r")
ax1.plot(-mu, 0, 'o', color="g")
ax1.plot(1 - mu, 0, 'o', color="b")
for i in range(Np):
    ax1.plot(LP[i, 0], LP[i, 1], 'o', color="k")

ax2.plot(U[:, 0], U[:, 1], '-', color="r")

ax2.plot(LP[selected_point_index - 1, 0], LP[selected_point_index - 1, 1], 'o', color="k")
fig.suptitle(f"Orbit around {lagrange_point_names} with Embedded RK")


ax1.set_title("Orbital view, with the solutions from integrating the Cauchy problem")
ax2.set_title("Lagrange point orbit")

for ax in fig.get_axes():
    ax.set(xlabel='x', ylabel='y')
    ax.grid()

plt.show()